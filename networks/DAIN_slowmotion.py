...
class DAIN_slowmotion(torch.nn.Module):
    def __init__(self,
        ...
        self.filterModule = FilterInterpolationModule.__dict__['interpolation']().cuda()
    ...
    def forward(self, input_0, input_2, padding):
        ...
        for temp_0,temp_1, timeoffset in zip(cur_offset_outputs[0], cur_offset_outputs[1], time_offsets):
            ...
            cur_output_temp         = cur_output_temp[:, :, padding[2]:padding[3], padding[0]: padding[1]]
            ref0                    = ref0[:, :, padding[2]:padding[3], padding[0]: padding[1]]
            ref2                    = ref2[:, :, padding[2]:padding[3], padding[0]: padding[1]]
            ctx0                    = ctx0[:, :, padding[2]:padding[3], padding[0]: padding[1]]
            ctx2                    = ctx2[:, :, padding[2]:padding[3], padding[0]: padding[1]]

            cur_output.append(cur_output_temp)

            rectify_input = torch.cat((cur_output_temp,ref0,ref2,
                                        cur_offset_output[0][:, :, padding[2]:padding[3], padding[0]: padding[1]],
                                        cur_offset_output[1][:, :, padding[2]:padding[3], padding[0]: padding[1]],
                                        cur_filter_output[0][:, :, padding[2]:padding[3], padding[0]: padding[1]],
                                        cur_filter_output[1][:, :, padding[2]:padding[3], padding[0]: padding[1]],
                                        ctx0,ctx2
                                        ),dim =1)
