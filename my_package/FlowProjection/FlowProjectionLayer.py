...
class FlowProjectionLayer(Module):
    ...
    def forward(self, input1, requires_grad = False):
        ...
        if input1.is_cuda :
            ...
            if self.count is None:
                #print(input1.type())
                if isinstance(input1, torch.cuda.FloatTensor):
                    self.count = torch.cuda.FloatTensor().resize_(input1.size(0), 1, input1.size(2), input1.size(3)).zero_()
                    self.output = torch.cuda.FloatTensor().resize_(input1.size()).zero_()
                else:
                    self.count = torch.cuda.HalfTensor().resize_(input1.size(0), 1, input1.size(2), input1.size(3)).zero_()
                    self.output = torch.cuda.HalfTensor().resize_(input1.size()).zero_()
            err = my_lib.FlowProjectionLayer_gpu_forward(input1, self.count, self.output, fillhole)
        else:
            output = torch.cuda.FloatTensor(input1.data.size())
            err = my_lib.FlowProjectionLayer_cpu_forward(input1, self.count, output, fillhole)
        if err != 0:
            print(err)
        # output = output/count # to divide the counter

        #self.save_for_backward(input1, self.count)
