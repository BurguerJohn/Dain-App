...
class PWCDCNet(nn.Module):
    ...
    """
    def __init__(self, md, cuda , width, height, batch):
        """
        ...
        xx = torch.arange(0, W_MAX).view(1,-1)
        yy = torch.arange(0, H_MAX).view(-1,1)
        ...
        #if RenderData.GetGlobalHalf():
        #    xx = xx.half()
        #    yy = yy.half()

        xx = xx.repeat(H_MAX,1)
        yy = yy.repeat(1,W_MAX)
    ...
    def warp(self, x, flo, index):
        ...
        mask = torch.autograd.Variable(x.resize_(x.size()).zero_() + 1, requires_grad = False)
        if self.use_cuda :
            mask = mask.cuda()
        #if RenderData.GetGlobalHalf():
        #    mask = mask.half()
    ...
    def forward(self,im1, im2, output_more = False):
        ...
        warp5 = self.warp(c25, up_flow6*0.625, 0)
        #if RenderData.GetGlobalHalf():
        #    warp5 = warp5.half()
        ...
        warp4 = self.warp(c24, up_flow5*1.25, 1)
        #if RenderData.GetGlobalHalf():
        #    warp4 = warp4.half()
        ...
        warp3 = self.warp(c23, up_flow4*2.5, 2)
        #if RenderData.GetGlobalHalf():
        #    warp3 = warp3.half()
        ...
        warp2 = self.warp(c22, up_flow3*5.0, 3)
        #if RenderData.GetGlobalHalf():
        #    warp2 = warp2.half()
...
def pwc_dc_net(path=None, cuda=True, size = [2048, 1024], batch = 1):

    model = PWCDCNet(4, cuda, size[0], size[1], batch)
    if path is not None:
